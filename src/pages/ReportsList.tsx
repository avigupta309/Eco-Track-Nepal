...
import { Search, TreePine, Mountain, Droplets, Trash2, Flame, AlertCircle, Calendar, MapPin } from 'lucide-react';
...
export default function ReportsList({ reports, onNavigate, onSelectReport }: ReportsListProps) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-green-800 mb-2">Environmental Reports</h1>
          <p className="text-stone-600">Browse all reported environmental issues across Nepal</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-green-50 border-b border-stone-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-stone-700">Location</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-stone-700">Reporter</th>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className={`${categoryColors[report.category]} p-2 rounded-lg`}>
                            <Icon className="w-4 h-4 text-white" />
                          </div>
                          <span className="font-medium text-stone-800">{report.category}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-stone-600 line-clamp-2 max-w-md">{report.description}</p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-stone-800">{report.reporter.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-1 text-stone-600">
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-3 mb-3">
                    <div className={`${categoryColors[report.category]} p-2 rounded-lg flex-shrink-0`}>
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-semibold text-stone-800 mb-1">{report.category}</h3>
                      <p className="text-sm text-stone-600 line-clamp-2 mb-2">{report.description}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center space-x-2 text-stone-600">
                      <MapPin className="w-4 h-4" />
                      <span>{report.district}, {report.province}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-stone-600">{report.reporter.name}</span>
                      </div>
          ...
          {filteredReports.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-stone-500 mt-1">Try adjusting your search terms</p>
            </div>
          )}
        </div>
